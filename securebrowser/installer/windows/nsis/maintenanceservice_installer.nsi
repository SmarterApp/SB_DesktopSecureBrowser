...
; this installed.
!define FallbackKey \
  "SOFTWARE\AIR\MaintenanceService\3932ecacee736d366d6436db0f55bce4"

!define CompanyName "AIR"
...
!define MaintFullName "AIR Maintenance Service"
!undef BrandFullName
...
InstallDirRegKey HKLM "Software\AIR\MaintenanceService" ""
...
!define MaintUninstallKey \
 "Software\Microsoft\Windows\CurrentVersion\Uninstall\AIRMaintenanceService"
...
Section "MaintenanceService"
  ...
  WriteRegStr HKLM "${MaintUninstallKey}" "Publisher" "AIR"
  ...
  ${If} ${RunningX64}
    SetRegView 64
  ${EndIf}
  WriteRegDWORD HKLM "Software\AIR\MaintenanceService" "Attempted" 1
  WriteRegDWORD HKLM "Software\AIR\MaintenanceService" "Installed" 1
  DeleteRegValue HKLM "Software\AIR\MaintenanceService" "FFPrefetchDisabled"

  ; Included here for debug purposes only.  
  ; These keys are used to bypass the installation dir is a valid installation
  ; check from the service so that tests can be run.
  ; WriteRegStr HKLM "${FallbackKey}\0" "name" "AIR Corporation"
...
Section "Uninstall"
  ...
  Push "$APPDATA\AIR\logs\maintenanceservice.log"
  Call un.RenameDelete
  Push "$APPDATA\AIR\logs\maintenanceservice-1.log"
  Call un.RenameDelete
  Push "$APPDATA\AIR\logs\maintenanceservice-2.log"
  Call un.RenameDelete
  Push "$APPDATA\AIR\logs\maintenanceservice-3.log"
  Call un.RenameDelete
  Push "$APPDATA\AIR\logs\maintenanceservice-4.log"
  Call un.RenameDelete
  Push "$APPDATA\AIR\logs\maintenanceservice-5.log"
  Call un.RenameDelete
  Push "$APPDATA\AIR\logs\maintenanceservice-6.log"
  Call un.RenameDelete
  Push "$APPDATA\AIR\logs\maintenanceservice-7.log"
  Call un.RenameDelete
  Push "$APPDATA\AIR\logs\maintenanceservice-8.log"
  Call un.RenameDelete
  Push "$APPDATA\AIR\logs\maintenanceservice-9.log"
  Call un.RenameDelete
  Push "$APPDATA\AIR\logs\maintenanceservice-10.log"
  Call un.RenameDelete
  Push "$APPDATA\AIR\logs\maintenanceservice-install.log"
  Call un.RenameDelete
  Push "$APPDATA\AIR\logs\maintenanceservice-uninstall.log"
  Call un.RenameDelete
  RMDir /REBOOTOK "$APPDATA\AIR\logs"
  RMDir /REBOOTOK "$APPDATA\AIR"
  ...
  ${If} ${RunningX64}
    SetRegView 64
  ${EndIf}
  DeleteRegValue HKLM "Software\AIR\MaintenanceService" "Installed"
  DeleteRegValue HKLM "Software\AIR\MaintenanceService" "FFPrefetchDisabled"
